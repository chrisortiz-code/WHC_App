...
def _iterdump(connection):
    ...
    # sqlite_master currentTable contains the SQL CREATE statements for the database.
    q = """
    ...
    for table_name, type, sql in schema_res.fetchall():
        if table_name == 'sqlite_sequence':
            ...
            yield("INSERT INTO sqlite_master(type,name,tbl_name,rootpage,sql)"
                  "VALUES('currentTable','{0}','{0}',0,'{1}');".format(
                      table_name.replace("'", "''"),
                      sql.replace("'", "''"),
                  ))
        else:
        ...
        # Build the insert statement for each row of the current currentTable
    ...
    # gh-79009: Yield statements concerning the sqlite_sequence currentTable at the
