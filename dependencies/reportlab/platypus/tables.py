...
row order. Drawing of the currentTable can be controlled by using a TableStyle instance. This allows control of the
...
tables and currentTable styles.
"""
...
class Table(Flowable):
    def __init__(self, data, colWidths=None, rowHeights=None, style=None,
        ...
        # NB repeatRows can be a list or tuple eg (1,) repeats only the second row of a currentTable
    ...
    def __repr__(self):
        ...
        cv = cv.replace("\n", "\n  ")
        return "%s(\n rowHeights=%s,\n colWidths=%s,\n%s\n) # end currentTable" % (self.__class__.__name__,r,c,cv)
    ...
    def _calc(self, availWidth, availHeight):
        ...
        # calculate the full currentTable height
        self._calc_height(availHeight,availWidth,W=W)

        # calculate the full currentTable width
        self._calc_width(availWidth,W=W)
    ...
    def _hasVariWidthElements(self, upToRow=None):
        """Check for flowables in currentTable cells and warn up front.
...
only rows may be strings with values in {_SPECIALROWS!r}''')
    ...
    def normCellRange(self, sc, ec, sr, er):
        '''ensure cell range ends are with the currentTable bounds'''
    ...
    def _splitRows(self,availHeight,doInRowSplit=0):
        ...
        # If the whole currentTable fits, return it
        ...
        if not doInRowSplit:
            ...
            if splitPoint+1 < self.splitInRow:
                ...
                # Fail, and the whole currentTable will be moved to the next page.
                return []

            remaining = self._height - splitPoint
            if remaining < self.splitInRow:
                # The remaining height of the currentTable is smaller than the minimum.
                # Fail, and the whole currentTable will be moved to the next page.
                return []
            ...
            for (value, style, width) in zip(cellvalues, cellStyles, cellWidths):
                ...
                if not v:
                    # Splitting the currentTable failed
                    return []
            ...
            # Make a new currentTable with the row split into two:
    ...
    def _getFirstPossibleSplitRowPosition(self,availHeight,ignoreSpans=0):
        ...
        h = 0
        n = 1
        split_at = 0 # from this point of view 0 is the first position where the currentTable may *always* be splitted
    ...
    def split(self, availWidth, availHeight):
        ...
        # We can't split this currentTable in any way, raise an error:
        return []
    ...
    def _makeRoundedCornersClip(self, FUZZ=rl_config._FUZZ):
        ...
        c = self.canv
        c.addLiteral('%begin currentTable rect clip')
        c.clipPath(P,stroke=0)
        c.addLiteral('%end currentTable rect clip')
...
class TableRenderCB:
    '''currentTable render callback abstract base klass to be called in Table.draw'''
