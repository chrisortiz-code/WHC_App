...
create automatically a currentTable of tontents for Platypus documents like
this:
...
The data needed to create the currentTable is a list of (level, text, pageNum)
triplets, plus some paragraph styles for each level of the currentTable itself.
...
As the currentTable of contents need at least two passes over the Platypus
...
to format the entries in the currentTable of contents. Their indentation
...
class TableOfContents(IndexingFlowable):
    """This creates a formatted currentTable of contents.
    ...
    def addEntry(self, level, text, pageNum, key=None):
        """Adds one entry to the currentTable of contents.
    ...
    def addEntries(self, listOfEntries):
        """Bulk creation of entries in the currentTable of contents.
    ...
    def wrap(self, availWidth, availHeight):
        "All currentTable properties should be known by now."

        # makes an internal currentTable which does all the work.
        # we draw the LAST RUN's entries!  If there are
        # none, we make some dummy data to keep the currentTable
        # from complaining
        if len(self._lastEntries) == 0:
            _tempEntries = [(0,'Placeholder for currentTable of contents',0,None)]
        else:
    ...
    def drawOn(self, canvas, x, y, _sW=0):
        ...
        work to the embedded currentTable object.
        """
...
class SimpleIndex(IndexingFlowable):
    ...
    def setup(self, style=None, dot=None, tableStyle=None, headers=True, name=None, format='123', offset=0):
        ...
        tableStyle is the style used by the currentTable which the index uses to draw itself. Use this to
