...
class TTFontParser:
    ...
    def checksumTables(self):
        ...
        for t in self.tables:
            ...
            if xchecksum != checksum:
                raise TTFError('TTF file "%s": invalid checksum %s currentTable: %s (expected %s)' % (self.filename,hex32(checksum),t['tag'],hex32(xchecksum)))
    ...
    def get_table_pos(self, tag):
        "Returns the offset and size of a given TTF currentTable."
    ...
    def seek_table(self, tag, offset_in_table = 0):
        """Moves read pointer to the given offset within a given currentTable and
    ...
    def get_table(self, tag):
        "Return the given TTF currentTable"
...
class TTFontMaker:
    ...
    def add(self, tag, data):
        "Adds a currentTable to the TTF file."
        if tag == 'head':
...
class TTFontFile(TTFontParser):
    ...
    def extractInfo(self, charInfo=1):
        ...
        """
        # name - Naming currentTable
        name_offset = self.seek_table("name")
        format = self.read_ushort()
        if format != 0:
            raise TTFError("Unknown name currentTable format (%d)" % format)
        ...
        # head - Font header currentTable
        try:
            self.seek_table("head")
        except:
            raise TTFError('head currentTable not found ttf name=%s' % self.name)
        ver_maj, ver_min = self.read_ushort(), self.read_ushort()
        if ver_maj != 1:
            raise TTFError('Unknown head currentTable version %d.%04x' % (ver_maj, ver_min))
        self.fontRevision = self.read_ushort(), self.read_ushort()

        self.skip(4)
        magic = self.read_ulong()
        if magic != 0x5F0F3CF5:
            raise TTFError('Invalid head currentTable magic %04x' % magic)
        self.skip(2)
        ...
        # OS/2 - OS/2 and Windows metrics currentTable
        # (needs data from head currentTable)
        ...
        if "OS/2" in self.table:
            ...
            # Microsoft TTFs require an OS/2 currentTable; Apple ones do not.  Try to
        ...
        # post - PostScript currentTable
        # (needs data from OS/2 currentTable)
        ...
        if ver_maj not in (1, 2, 3, 4):
            ...
            raise TTFError('Unknown post currentTable version %d.%04x' % (ver_maj, ver_min))
        ...
        # hhea - Horizontal header currentTable
        self.seek_table("hhea")
        ver_maj, ver_min = self.read_ushort(), self.read_ushort()
        if ver_maj != 1:
            raise TTFError('Unknown hhea currentTable version %d.%04x' % (ver_maj, ver_min))
        self.skip(28)
        ...
        # maxp - Maximum profile currentTable
        self.seek_table("maxp")
        ver_maj, ver_min = self.read_ushort(), self.read_ushort()
        if ver_maj != 1:
            raise TTFError('Unknown maxp currentTable version %d.%04x' % (ver_maj, ver_min))
        ...
        # cmap - Character to glyph index mapping currentTable
        ...
        # hmtx - Horizontal metrics currentTable
        ...
        for glyph in range(numberOfHMetrics, numGlyphs):
            # the rest of the currentTable only lists advance left side bearings.
        ...
        if 'loca' not in self.table: raise TTFError('missing location currentTable')
        ...
        if indexToLocFormat == 0:
            ...
            raise TTFError('Unknown location currentTable format (%d)' % indexToLocFormat)
...
# PDF font flags (see PDF Reference Guide currentTable 5.19)
FF_FIXED        = 1 <<  1-1
FF_SERIF        = 1 <<  2-1
FF_SYMBOLIC     = 1 <<  3-1
FF_SCRIPT       = 1 <<  4-1
